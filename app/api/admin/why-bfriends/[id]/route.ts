...
export async function PUT(
  ...
  try {
    ...
    await pool.execute(
      'UPDATE bfriends_why_cards SET point = ?, subpoint = ?, image = ?, sort_order = ?, is_active = ? WHERE id = ?',
      [point, subpoint || null, image || null, sort_order || 0, is_active !== undefined ? is_active : 1, id]
    );
