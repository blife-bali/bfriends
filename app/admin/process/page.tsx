...
interface ProcessStep {
  ...
  image: string; sort_order: number; is_active: number;
  subpoints?: Subpoint[];
}
...
export default function ProcessPage() {
  ...
  return (
    <AdminLayout title="Process Steps" username={username}>
      <div className="admin-card">
        <div className="admin-card-header">
          <h2>Process / Customer Journey</h2>
          <button onClick={() => setEditing({ ...empty, subpoints: [] })} className="admin-btn admin-btn-primary">+ Tambah Step</button>
        </div>
        <DataTable
          columns={[
            { key: 'number', label: '#' },
      ...
      {editing && (
        <div className="admin-modal-overlay" onClick={() => setEditing(null)}>
          <div className="admin-modal" style={{ width: 700, maxHeight: '85vh' }} onClick={(e) => e.stopPropagation()}>
            <h3>{editing.id ? 'Edit Step' : 'Tambah Step'}</h3>
      ...
      {deleteTarget && <ConfirmDialog message={`Hapus step "${deleteTarget.title}"?`} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} />}
